...
@st.cache_resource
def get_neural_searcher(collection_name="legal_collection"):
        ...
        if uploaded_file is not None:
            ...
            if bytes_data is not None:
                ...
                if click_search_ocr:
                    ...
                    result = neural_searcher.search(legal_info.__str__())
                    df = pd.DataFrame([item[0] for item in result])
                    df['value'] = [item[1] for item in result]
                    st.write('Results: ')
                    st.dataframe(df)
